...
export const AddEditModal: React.FC<TimerModalProps> = ({ isOpen, onClose, timer, mode }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!validateTimerForm({ title, description, hours, minutes, seconds })) {
      showToast({ message: "Please fill in all required fields correctly!", type: "error" });
      return;
    }
      ...
      <div className="bg-white rounded-lg p-6 w-full max-w-md shadow-xl">
        <div className="flex justify-between items-center mb-6">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-xl font-semibold">{mode === 'add' ? 'Add New Timer' : 'Edit Timer'}</h2>
          </div>
          <button 
            onClick={handleClose}
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            ...
            <input
              ...
              maxLength={50}
              className={`w-full px-4 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 transition-colors ${
          ...
          <div>
            ...
            <textarea
              ...
              rows={3}
              className="w-full px-4 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none transition-colors border-gray-300 placeholder-gray-500"
